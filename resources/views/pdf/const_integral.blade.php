...
<thead>
  <tr>
    ...
    <th><strong>Asignación</strong></th>
  </tr>
</thead>
<tbody>
  @foreach ($detalles as $detalle)
    <tr>
    <td>{{ $detalle->cod_concep }}</td>
    <td style="text-align:justify;">{{ $detalle->descripcion }}</td>
    <td style="text-align:right;">{{ number_format($detalle->total_a, 2, ',', '.') }}</td>
    </tr>
  @endforeach
</tbody>
<tfoot>
      <tr>
        <th></th>
          ...
          <th style="text-align:right;">{{ number_format($footer['total'], 2, ',', '.') }}</th>   
      </tr>
      <tr>
          <th colspan="3" style="text-align:center;">{{ $footer['letras'] }}</th> 
      </tr>
</tfoot>
</table>
</p>
